...
// DefaultGHCLICACertPath is the default container path where the CA certificate
// is copied so the gh CLI trusts the proxy's self-signed TLS certificate.
const DefaultGHCLICACertPath = "/opt/dcx/gh-proxy/ca.crt"

// DefaultGHCLIAPIURL is the default GitHub API URL that the proxy forwards
// requests to.
const DefaultGHCLIAPIURL = "https://api.github.com"

// DefaultGHCLICertExpiry is the default validity duration for the self-signed
// TLS certificates generated by the proxy. The certificates are ephemeral —
// they only need to last for the duration of a dcx exec session.
const DefaultGHCLICertExpiry = 24 * time.Hour
